...
#define ID_LED_SLAVE_OFF   0x02  //LED on the slave side
...
void LIN0_Oscillator_Init (void);           // Configure the system clock
void LIN0_Port_Init (void);                 // Configure the Crossbar and GPIO
void LIN0_PCA0_Init (void);                 // Configure the PCA
...
//-----------------------------------------------------------------------------
// MAIN Routine ==> renamed to: Launch_Example_Master_Application
//-----------------------------------------------------------------------------
...
void Launch_Example_Master_Application (void)
{
   ...
   LIN0_Oscillator_Init ();                 // Initialize the system clock
   LIN0_Port_Init ();                       // Initialize crossbar and GPIO
   LIN0_PCA0_Init ();                       // Initialize PCA0
...
//
//-----------------------------------------------------------------------------
void LIN0_Oscillator_Init (void)
{
   U8 SFRPAGE_save = SFRPAGE;
   SFRPAGE = CONFIG_PAGE;
...
//-----------------------------------------------------------------------------
// LIN0_Port_Init *EDIT THIS SECTION*
//-----------------------------------------------------------------------------
//
// Return Value : None
// Parameters   : None
...
//
//-----------------------------------------------------------------------------
void LIN0_Port_Init (void)
{
   U8 SFRPAGE_save = SFRPAGE;
   SFRPAGE = CONFIG_PAGE;

   P1MDOUT   = 0x08;                   // Set P1.1 to open-drain                                      
                                       // Set P1.3 to push-pull
                                       // Set P1.4 to open-drain

   #if LIN_TX_PUSH_PULL == 1 
...
INTERRUPT (PCA0_ISR, INTERRUPT_PCA0)
{
   ...
   if (CCF2)
   {
      ...
      {
         sw1now = SW1; // Read current state of switch

         if ((sw1prev == 0) && (sw1now == 1))
         {
         ...
         sw1prev = sw1now;                   // Save value of switch
      }
   }
   else
   {
