...
U8 SRC_Get_Info (U8 *src_info)
{
   ...
   while (Tx_Complete_Flag == 0);      // Wait for Tx complete

   while (Rx_Complete_Flag == 0);      // Wait for Rx complete
   ...
   // [7] CAN Device Address
   // [8] Application FW Start Addr Low (0)
   // [9] Application FW Start Addr (1)
   // [10] Application FW Start Addr High (2)
   // [11] Application FW Start Addr Low (0)
   // [12] Application FW Start Addr (1)
   // [13] Application FW Start Addr High (2)
...
U8 SRC_Get_Page_Info (U32 *addr, U16 *src_page_crc)
{
   ...
   *addr = ((*(Rx_Buf_Ptr+3)) << 16) | ((*(Rx_Buf_Ptr+2)) << 8) | (*(Rx_Buf_Ptr+1));
